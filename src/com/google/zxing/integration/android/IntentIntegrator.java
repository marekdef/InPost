/*
 * Copyright 2009 ZXing authors Licensed under the Apache License, Version 2.0 (the
...
/**
 * <p>
 * A utility class which helps ease integration with Barcode Scanner via {@link Intent}s.
 * This is a simple way to invoke barcode scanning and receive the result, without any
 * need to integrate, modify, or learn the project's source code.
 * </p>
 ...
 * {@link #initiateScan()} method will prompt the user to download the application, if
 * needed.
 * </p>
 * <p>
 * There are a few steps to using this integration. First, your {@link Activity} must
 * implement the method {@link Activity#onActivityResult(int, int, Intent)} and include a
 ...
 * </p>
 * <p>
 * Second, just call this in response to a user action somewhere to begin the scan
 * process:
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * Note that {@link #initiateScan()} returns an {@link AlertDialog} which is non-null if
 * the user was prompted to download the application. This lets the calling app
 * potentially manage the dialog. In particular, ideally, the app dismisses the dialog if
 * it's still active in its {@link Activity#onPause()} method.
 * </p>
 * <p>
 * You can use {@link #setTitle(String)} to customize the title of this download prompt
 * dialog (or, use {@link #setTitleByID(int)} to set the title by string resource ID.)
 * Likewise, the prompt message, and yes/no button labels can be changed.
 * </p>
 * <p>
 * By default, this will only allow applications that are known to respond to this intent
 * correctly do so. The apps that are allowed to response can be set with
 * {@link #setTargetApplications(Collection)}. For example, set to
...
public class IntentIntegrator
{
     ...
     * corresponding to their names in ZXing's {@code BarcodeFormat} class like "UPC_A".
     * You can supply constants like {@link #PRODUCT_CODE_TYPES} for example.
     */
     ...
     * This method is defined to allow different methods of activity starting for newer
     * versions of Android and for compatibility library.
     * @param intent
     * Intent to start.
     * @param code
    ...
    /**
     * Shares the given text by encoding it as a barcode, such that another user can scan
     * the text off the screen of the device.
     * @param text
